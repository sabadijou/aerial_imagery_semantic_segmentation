...
class PixelCL(nn.Module):
    def __init__(
        ...
        self.patch = transforms.RandomErasing(p=.8,
                                            scale=(0.03, 0.03),
                                            ratio=(0.5, 3.33),
                                            value='random',
                                            inplace=False)
    ...
    def forward(self, x, return_positive_pairs = False):
        shape, device, prob_flip = x.shape, 'cuda', self.prob_rand_hflip
        ...
        ###############################################################################
        # Patch Augmentation
        choice_ = random.choice([True, False])
        if choice_:
            x1 = self.patch_augmentation(x)
            x2 = x
        else:
            x2 = self.patch_augmentation(x)
            x1 = x
        ###############################################################################
        cutout_coordinates_one, _ = cutout_coordinates(x1, self.cutout_ratio_range)
        cutout_coordinates_two, _ = cutout_coordinates(x2, self.cutout_ratio_range)

        image_one_cutout = cutout_and_resize(x1, cutout_coordinates_one, mode=self.cutout_interpolate_mode)
        image_two_cutout = cutout_and_resize(x2, cutout_coordinates_two, mode=self.cutout_interpolate_mode)

        image_one_cutout = flip_image_one_fn(image_one_cutout)
        image_two_cutout = flip_image_two_fn(image_two_cutout)

        image_one_cutout, image_two_cutout = self.augment1(image_one_cutout.float().permute(0, 3, 1, 2)), \
            self.augment2(image_two_cutout.float().permute(0, 3, 1, 2))
        ...
        return ret, pix_loss, instance_loss